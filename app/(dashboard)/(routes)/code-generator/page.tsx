...
const onSubmit = async (formData: z.infer<typeof codeGeneratorFormSchema>) => {
      try {
  ...
  return (
    <div>
      <Heading title="AI Code Generator" 
            ...
            <FormField name='prompt' render={({field})=>(
              <FormItem className='col-span-12 lg:col-span-10'>
                <FormControl className='m=0 p-0'>
                  ...
                  placeholder='Simple landing page template using Bootstrap 5' {...field} />
                </FormControl>
              </FormItem>
            )} 
            />
            ...
            {messages.map((message, index) => (
              ...
              message.role === "assistant" ? "bg-white border border-zinc-800/10" : "bg-muted")}>
                ...
                <p className='text-sm'>
                  <ReactMarkdown 
                  components={{
                    pre: ({ node, ...props }) => (<div className="overflow-x-auto rounded-lg bg-muted/10 p-4">
                      <pre {...props} />
                    </div>),
